...
<head>
    <meta charset="UTF-8">
    <title>페이징 + 검색</title>
	<link rel="stylesheet" type="text/css" href="../css/main_board_listPageSearch.css">
</head>
<body>
    <div id="nav">
        <%@ include file="../include/nav.jsp"%>
    ...
    <!-- 검색 폼 추가 -->
    <div class="search">
        <form method="get" action="/board/listPageSearch">
            <select name="searchType">
                <option value="title" <c:if test="${searchType eq 'title'}">selected</c:if>>제목</option>
                <option value="content" <c:if test="${searchType eq 'content'}">selected</c:if>>내용</option>
                <option value="title_content" <c:if test="${searchType eq 'title_content'}">selected</c:if>>제목+내용</option>
            ...
            <input type="text" name="keyword" value="${keyword}">
            <input type="hidden" name="num" value="1"> <!-- 기본 페이지 번호 -->
